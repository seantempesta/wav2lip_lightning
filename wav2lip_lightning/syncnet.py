    ...
    def __getitem__(self, idx):
        while 1:
            ...
            try:
                ...
                fpspath = join(vidname, "fps.txt")
                with open(fpspath, "r") as text_file:
                    video_fps = float(text_file.readline())
...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        while 1:
            idx = random.randint(0, len(self.all_videos) - 1)
            vidname = self.all_videos[idx]
            reader = torchvision.io.VideoReader(vidname, "video")
...
class SyncNet_color(pl.LightningModule):
    ...
    def __init__(self, learning_rate=1e-2):
        ...
        self.face_encoder = nn.Sequential(
            Conv2d(20, 32, kernel_size=(7, 7), stride=1, padding=3), # input is (RGB + face depth mesh) * 5 images = 20 channels
...
def repl_test():
    ...
    parser.add_argument('--data_root', default="./sample_data/avaspeech-orig")
    parser = pl.Trainer.add_argparse_args(parser)
    args = parser.parse_args()

    # Dataset and Dataloader setup
    train_dataset = Dataset('train_orig', args)
    ...
    model = SyncNet_color()
    pretrained_dict = torch.load('./lipsync_expert.pth', map_location=torch.device('cpu'))['state_dict']
...
def debug():
    ...
    face_output = model.face_encoder(face_sequences)
    ...
    img = Image.open('./sample_data/alpha.png')
    pix = np.array(img)
    plt.imshow(pix)
    plt.show()